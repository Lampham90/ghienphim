...
const MovieCard = memo(({ movie, variant = 'vertical', index = 0, priority = false, isDragging = false }: MovieCardProps) => {
  ...
  return (
    <div className={`${isSpecial ? 'min-w-[170px] md:min-w-[calc(100%/5.5)]' : 'min-w-[160px] md:min-w-[calc(100%/7)]'} snap-start group relative flex flex-col items-center pt-4 ${isDragging ? 'pointer-events-none' : ''}`}>
      <Link href={`/phim/${movie.slug}?poster=${encodeURIComponent(rawPoster)}&thumb=${encodeURIComponent(rawThumb)}`} 
      ...
      <div className={`mt-4 px-2 ${isSpecial ? 'w-full text-left' : 'w-full text-center'}`}>
        {isSpecial ? (
          <div className="mt-1 flex gap-3 items-start pr-2">
            <span className="text-[35px] md:text-[50px] font-black italic leading-[0.8] text-red-600 transition-transform">{index + 1}</span>
            <div className="flex flex-col pt-1 w-full">
              ...
              <p className="text-[8px] font-bold text-white/30 uppercase mt-1 italic">{movie.country}</p>
            </div>
          </div>
        ) : (
          <h3 className="text-[10px] font-black uppercase text-white/80 group-hover:text-red-500 transition-colors italic line-clamp-2 leading-tight">{movie.name}</h3>
        )}
      </div>
    </div>
  );
